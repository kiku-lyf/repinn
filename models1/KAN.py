...
#
#
#     def forward(self, x_res, y_res, t_res):
#         '''
#         KAN forward
#
#         Args:
#         -----
#             x_res : 2D torch.float
#                 x coordinate inputs, shape (batch, 1)
#             y_res : 2D torch.float
#                 y coordinate inputs, shape (batch, 1)
#             t_res : 2D torch.float
#                 t coordinate inputs, shape (batch, 1)
#
#         Returns:
#         --------
...
#         Example
#         -------
#         >>> model = Model(width=[3,5,1], grid=5, k=3, noise_scale=1.0)
#         >>> x = torch.normal(0,1,size=(100,1))
#         >>> y = torch.normal(0,1,size=(100,1))
#         >>> t = torch.normal(0,1,size=(100,1))
#         >>> model(x, y, t).shape
#         torch.Size([100, 1])
#         '''
#         # 将x, y, t三个维度拼接在一起
#         x = torch.cat([x_res, y_res, t_res], dim=-1)
#
...
#     #     Args:
#     #     -----
#     #         coords : 2D or 3D torch.float
#     #             inputs, shape (batch, input dimension) for standard models
#     #                     or (batch, sequence_length, input dimension) for sequence models
#     #
#     #     Returns:
#     #     --------
#     #         y : 2D or 3D torch.float
#     #             outputs, shape (batch, output dimension) or (batch, sequence_length, output dimension)
#     #
#     #     Example
#     #     -------
#     #     >>> model = Model(width=[3,5,2], grid=5, k=3)
...
#     #
#     #     x = coords
#     #     self.acts.append(x)  # acts shape: (batch, width[l]) or (batch, seq_len, width[l])
#     #
#     #     for l in range(self.depth):
#     #
#     #         x_numerical, preacts, postacts_numerical, postspline = self.act_fun[l](x)
...
#     #             grid_reshape = self.act_fun[l].grid.reshape(self.width[l + 1], self.width[l], -1)
#     #             input_range = grid_reshape[:, :, -1] - grid_reshape[:, :, 0] + 1e-4
#     #             output_range = torch.mean(torch.abs(postacts), dim=(0, 1))  # average over batch and seq
#     #         else:  # (batch, features)
#     #             grid_reshape = self.act_fun[l].grid.reshape(self.width[l + 1], self.width[l], -1)
#     #             input_range = grid_reshape[:, :, -1] - grid_reshape[:, :, 0] + 1e-4
#     #             output_range = torch.mean(torch.abs(postacts), dim=0)
#     #
#     #         self.acts_scale.append(output_range / input_range)
#     #         self.acts_scale_std.append(torch.std(postacts.reshape(-1, *postacts.shape[-2:]), dim=0))
#     #         self.spline_preacts.append(preacts.detach())
#     #         self.spline_postacts.append(postacts.detach())
#     #         self.spline_postsplines.append(postspline.detach())
#     #
...
class Model(nn.Module):
    ...
    def train(self, dataset=None, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0.,
              lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50,
              batch=-1,
              small_mag_threshold=1e-16, small_reg_factor=1., metrics=None, sglr_avoid=False, save_fig=False,
              in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video', device='cpu'):
        ...
        -----
            dataset : dic or bool
                if bool, use PyTorch's train/eval mode switching (dataset=True means train mode, False means eval mode)
                else contains dataset['train_input'], dataset['train_label'], dataset['test_input'], dataset['test_label']
        '''
        # 处理 PyTorch 的 train(mode) 调用
        if isinstance(dataset, bool):
            # 如果第一个参数是 bool，说明是 PyTorch 的模式切换调用
