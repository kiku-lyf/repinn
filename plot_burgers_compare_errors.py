#!/usr/bin/env python3
"""
同时对比多个模型在Burgers方程上的误差热图

特性：
- 支持传入多个模型名与模型权重路径
- 支持从.mat文件加载真实解数据，或使用解析解/数值解
- 统一色标范围，便于横向比较
- 标题中显示相对L1/L2误差
"""
...
def burgers_analytical(x, t, nu=0.01/np.pi):
    """
    Burgers方程的Cole-Hopf解析解（如果适用）
    对于特定的初始条件：u(x,0) = -sin(πx)
    
    注意：这个解析解仅适用于特定初始条件和边界条件
    如果您的Burgers方程使用不同的初始条件，请使用数值参考解或从文件加载
    """
    # 这是一个简化的解析解形式，实际应用中可能需要根据具体初始条件调整
    # 如果没有合适的解析解，应该从文件加载数值参考解
    raise NotImplementedError("Burgers方程通常需要数值参考解，请使用--data_path参数从文件加载")


def load_burgers_data(data_path=None):
    """
    加载Burgers方程的真实解数据
    ...
    try:
        ...
        # 根据用户示例，使用 meshgrid(t_star, x_star)
        # 根据用户示例，使用 meshgrid(t_star, x_star)
...
def main():
    parser = argparse.ArgumentParser('Compare multiple models: Burgers errors')
    ...
    parser.add_argument('--data_path', type=str, default="./burgers.mat",
                        help='Burgers真实解数据文件路径（.mat格式，必需）')
    ...
    parser.add_argument('--filename', type=str, default='burgers_models_errors.pdf')
    ...
    if args.model_paths is None or len(args.model_paths) == 0:
        model_paths = [f'./results/burgers_{m}_point.pt' for m in models]
    else:
    ...
    # 尝试加载真实解数据
    exact_data, XX_data, TT_data, x_star_data, t_star_data = load_burgers_data(args.data_path)
    ...
    if exact_data is not None:
        ...
        if args.data_path is None:
            print('错误: Burgers方程需要参考解数据，请使用--data_path参数指定.mat文件')
            print('Burgers方程通常没有简单的解析解，需要使用数值参考解')
            return
        else:
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # 现在 err.shape == (len(x_star_ref), len(t_star_ref))
            im = ax.pcolormesh(t_star_ref, x_star_ref, err, cmap='jet', shading='auto', vmin=0.0, vmax=vmax)

            ax.set_xlabel('$t$')
            ax.set_ylabel('$x$')
            ax.set_title(f'Burgers ({name}) | L1: {rl1:.2e}, L2: {rl2:.2e}')
            idx += 1
